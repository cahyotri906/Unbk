...
$sql4 = mysql_query("SELECT * from cbt_ujian WHERE XTokenUjian='$token'");
while($nil= mysql_fetch_array($sql4)){ 
...
if(isset($_REQUEST['upl'])){ 
	$sqlnilai = mysql_query("select * from cbt_nilai where XTokenUjian = '$_REQUEST[token]' AND XStatus='0' or XStatus='1'");
	
	while($nilai=mysql_fetch_array($sqlnilai)){
		include "../../config/server_pusat.php";
		$cek = mysql_num_rows(mysql_query("select * from cbt_nilai where  XTokenUjian = '$_REQUEST[token]' and XNomerUjian='$nilai[XNomerUjian]'"));
		if($cek==0){
		$sql = mysql_query("insert into cbt_nilai (XNomerUjian,XNIK,XKodeUjian,XTokenUjian,XTgl,XJumSoal,XBenar,XSalah,XNilai,XPersenPil,XPersenEsai,
	...
	include "../../config/server.php";
	$sqljawab = mysql_query("update cbt_nilai SET XStatus='9' where XNomerUjian = '$nilai[XNomerUjian]' and  XTokenUjian = '$_REQUEST[token]'");
...
<button type="button" class="btn btn-success"><i class='fa fa-refresh fa-fw'></i></button></a>
                        ...
                        <div class="panel-body">
                            <table width="100%" class="table table-striped table-bordered table-hover">
                                ...
                                <?php   
								include "../../config/server.php";
								$sql5 = mysql_query("SELECT * from cbt_nilai
								WHERE XTokenUjian='$token'");
								
								$nom = 1;	
								
								while($nil= mysql_fetch_array($sql5)){ 
