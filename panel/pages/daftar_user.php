...
if ($_REQUEST["txt_login"] >1){$pass  =md5($_REQUEST['txt_pass']);}else{$pass  = md5($_REQUEST['txt_pass']);}
	if($_REQUEST['txt_pass']==""){
		$sql = mysql_query("update cbt_user set login='$_REQUEST[txt_login]', Username = '$_REQUEST[txt_usern]', NIP = '$_REQUEST[txt_nip]', 
...
$sqlcek = mysql_num_rows(mysql_query("select * from cbt_user where Username = '$_REQUEST[txt_usern]'"));
	...
	echo "<script type='text/javascript'>alert('$message');</script>";
		...
		$sql = mysql_query("insert into cbt_user (Username, Password, NIP, Nama, Alamat, HP, Email, login, Status,XPoto) values  
